Here you will find the files to reproduce the ISRF of the MMA chemical evolution model (Millán-Irigoyen et. al. 2020) shown in Romero et al. (in prep), section 3.

-MMA_positions.txt contains the positions where the mean intensity output will be saved.
-MMA_gas.dat contains the initial conditions of the gas. The files Mixclask will read the input spectra are in the first column, from the folder than contains 'main.py'.
-MMA_stars.dat contains the geometry and location of the stellar sources. Mixclask will look if the file in the first column exists in 'star_sources'.
-Inside the 'stars' folder, you will find the stellar spectra of all regions in 'MMA_stars.dat'.
...
This test can take some hours to complete, about 4-7h.

---

Once it is finished, I suggest to create a folder named 'MMAResults' (for instance, you may use other name) and move all folders called 'iteration' and any '.sed' files that are in the root folder.
If you want the average of all iterations, go to 'post_processing/averages.py'. Inside the file, change this line:
> inputs_path = '../MMAResults/'
And then run
> python3 averages.py
Inside 'MMAResults' folder, there should be another one called 'Statistics' with the average results.
